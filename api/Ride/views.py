...
class RideOffer(MethodView):
    ...
    def post(self, creator_id=2):
        ...
        if dest and date and time and meet_p and charges:
            ...
            if Ride.does_ride_exist(dest, time):
                return make_response(jsonify(
                    {'message': 'you are already offering a ride to this destination on this date'})), 409
            ride = Ride(dest, date, time, meet_p, charges)
            ride.save_ride((ride.destination,ride.date,ride.departure_time,ride.meetpoint,ride.charges,creator_id))
            return make_response(jsonify({'message': 'succesfully created ride'})), 201

        return make_response(jsonify(
            {'message': 'ensure you have provide all required details'}
        )), 400

    def delete(self, rideid, creator_id):
        '''cancel a ride offer'''
